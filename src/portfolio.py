...
class DriftTrade:
    def __init__(self):
        ...
        self.drift_http_url = loader.constants

    def get_markets(self):
        url = self.drift_http_url + "/v2/markets"
        response = requests.get(url)
        self.markets = response.json()
        return self.markets

    def get_positions(self, marketIndex=None, marketType=None):
        url = self.drift_http_url + "/v2/positions"
    ...
    def get_profit_n_loss(self):
        ...
        for position in self.positions["perp"]:
            temp_url = f"{self.drift_http_url}/v2/positionInfo/{position["marketIndex"]}"
            response = requests.get(temp_url)
            position_info = response.json()
            profit_n_loss += position_info["unrealizedPnl"] + position_info["unsettledPnl"]
